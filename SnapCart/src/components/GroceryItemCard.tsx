...
import { Minus, MinusIcon, Plus, ShoppingCart } from "lucide-react";
interface IGrocery {
    _id: string,
    name: string,
    category: string,
...
function GroceryItemCard({ item }: { item: IGrocery }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col p-3 cursor-pointer"
    >
...
>
  <button className='w-7 h-7 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 transition-all' onClick={()=>dispatch(decreaseQuantity(String(item._id)))}><Minus size={16} className='text-green-700'/></button>
  <span className='text-sm font-semibold text-gray-800'>{cartItem.quantity}</span>
  <button className='w-7 h-7 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 transition-all' onClick={()=>dispatch(increaseQuantity(String(item._id)))}><Plus size={16} className='text-green-700'/></button>
